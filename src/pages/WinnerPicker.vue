<template>
  <q-page class="flex flex-center bg-black">
    ...
    class="add-name column bg-red" >
      <q-card-section>
        <h2 class="deep-shadow text-center q-mt-none q-mb-md text-white text-bold">ÇEKİLİŞ</h2>
        <q-input
        v-model="namesText"
          filled
          autogrow
          type="textarea"
          bg-color="white"
          placeholder="Çekiliş İsimlerini Alt alta giriniz!"
          rows="10"
          cols="40"
    />
    ...
    <div v-else class="winner-screen">
      <h2
      v-if="pickedWinner" class="deep-shadow  text-white absolute-top text-center text-bold">KAZANAN</h2>
      <div
     ...
     v-if="currentName"
     dark
     :class="pickedWinner ? 'bg-green-7' : 'bg-negative'"
     class="bg-negative"
     bordered>
     <q-card-section>
...
export default {
  data () {
    return {
      ...
      currentName: "",
      pickedWinner: false,
      audioSrc: require('assets/drum.mp3'),
      zoomingNames:false
    }
  },
  methods: {
